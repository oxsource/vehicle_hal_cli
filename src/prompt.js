...
const createChoices = (object, length = 1) => {
    return Object.keys(object).filter(key => key != Format.KEY_MASK) .map(key => {
    ...
    const questions = [
        {
            type: 'input',
            name: 'name',
            message: 'VehiclePropertyName?(eg. HVAC_F_POWER_ON)',
            initial: property.name || '',
            validate: (e) => {return e.trim() !== '' || 'cannot be empty'},
        }
    ];
...
const propertyId = async (property) => {
    ...
    const questions = [
        {
            ...
            initial: Format.textHexInt(property.group || Types.VehiclePropertyGroup.VENDOR, 8)
        },
        {
            type: 'select',
            name: 'type',
            ...
            initial: Format.textHexInt(property.type || Types.VehiclePropertyType.INT32, 8)
        },
        {
            type: 'select',
            name: 'area',
        ...
        },
        {
            ...
            initial: Format.textHexInt(property.index || Types.VehiclePropertyIndex.INIT, 4),
            validate: (e) => {return Format.HEX_INT16_REGEX.test(e) || 'bad property index'},
        },
    ];
    ...
    property.id = Types.createPropertyId(value.group, value.type, value.area, value.index)
}
...
const propertyAccess = async (property) => {
    ...
    const perms = [answers.write.trim(), answers.read.trim()].map(key => Perms[key])
    if (perms[0].length > 0 && perms[0] === perms[1]) {
        perms[1] = '-'
    }
    property.perms = perms
    //property.perms decide property.access
    property.access = [Types.VehiclePropertyAccess.WRITE, Types.VehiclePropertyAccess.READ].map((e ,index) => {
...
const areaId = async (property, access = Types.VehiclePropertyAccess.READ) => {
    ...
    assert.ok(access == Types.VehiclePropertyAccess.READ || access || Types.VehiclePropertyAccess.WRITE, "not support area access type")
        ...
        const questions = [
            {
                ...
                validate: (e) => {return Format.HEX_INT16_REGEX.test(e) || 'bad global area'},
            },
        ];
        const answers = await prompt(questions)
        area.id = Types.createAreaId(Format.parseHexInt(answers.id))
    } else {
        const group = Object.keys(Types.VehicleArea).find(key => Types.VehicleArea[key] == type)
...
const areaConfig = async (area) => {
    ...
    const questions = [
        ...
        },
        {
            ...
            initial: area.pos || 0,
            validate: (e) => {return (e >= 0 && e < 64) || 'bad value'},
        },
        {
            type: 'number',
            name: 'size',
        ...
        },
        {
            type: 'input',
            name: 'mapping',
            message: 'CAN signal mapping(option)',
...
const areaMath = async (area) => {
    ...
    Object.keys(values).forEach(key => {area[key] = values[key]})
}
